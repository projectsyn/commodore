...
With https://todo_xref_maybe[SDD0025], we've introduced support for instantiating components multiple times.
...
Components declare that they support instantiation by exposing the field `_instance` in `parameters.<component_name>`.
...
Commodore also updates `parameters.<component_name>._instance` for each instance, so that this field always holds the instance name.

Let's take the configuration below, which includes component `nfs-client-provisioner` twice, once unaliased, and once aliased to `nfs-2`, as an example.
...
Components can use the field `_instance` to ensure objects don't collide, as that field is guaranteed to be unique to each instance.
