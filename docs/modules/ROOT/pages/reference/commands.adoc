...
If not defined otherwise through the `out` flag, the command will return the list of clusters in a human-readable table.

The command can also filter the output per tenant or sort the list by ID, tenant, or display name, using the `tenant` or `sort-by` flags.
