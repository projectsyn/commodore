...
+
In case, the Lieutenant API doesn't return the global Git repository URL, the global git base will also be used to determine URL of that repository.
It URL will be discovered as `GLOBAL_GIT_BASE/commodore-defaults.git`.
This behaviour is to remain backwards compatible with older versions of the Lieutenant API.
