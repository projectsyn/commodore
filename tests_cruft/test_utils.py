...
from commodore.cruft import exceptions
from commodore.cruft._commands import utils
...
def test_get_diff_with_unicode(project_dir):
    with pytest.raises(exceptions.ChangesetUnicodeError):
        utils.diff.get_diff(
            Path(project_dir, "tests_cruft", "testdata", "unicode-data").absolute(),
            Path(project_dir, "tests_cruft", "testdata", "non-unicode-data").absolute(),
        )
