...
# design options for source:
# * raw URL with placeholder for ARCH/OS (brittle, doesn't survive upstream changing URL format)
# * GitHub repo short form (projectsyn/jsonnet-bundler) / GitHub latest release
# API URL (probably not better than GitHub repo short form) + custom logic to
# download per tool (see below)
# * Use upstream's suggested install tooling / what we currently do for the container image
# what we need to do:
# * kustomize/jb: download correct artefact from GH release
# * helm: extract URL from release text and download from get.helm.sh
# or reuse logic from Dockerfile
# TODO(sg): refactor this so we can reuse the actual download/install logic for
#  `upgrade_tool()`. Alternatively consider folding upgrade into install.
